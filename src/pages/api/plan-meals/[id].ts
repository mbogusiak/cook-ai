...
export const PATCH: APIRoute = async (context) => {
  try {
    ...
    if (!validationResult.success) {
      ...
      return new Response(JSON.stringify({ error: "Invalid plan meal ID format" }), {
        status: 400,
    ...
    try {
      const updatedMeal = await updateMealStatus(context.locals.supabase, mealId, user.id, command.status);
      ...
      const { data: fullMeal, error: fetchError } = await context.locals.supabase
        .from("plan_meals")
        .select(
          "id, slot, status, calories_planned, portion_multiplier, portions_to_cook, multi_portion_group_id, is_leftover, recipe_id, updated_at"
        )
        .eq("id", mealId)
        .single();
      ...
      if (error instanceof ForbiddenError) {
        ...
        return new Response(JSON.stringify({ error: "You do not have permission to update this meal" }), {
          status: 403,
          headers: { "Content-Type": "application/json" },
        });
      }
