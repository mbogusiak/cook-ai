...
export function mapSupabaseAuthError(error: { message: string; status?: number }): AuthErrorResponse {
  ...
  if (errorMessage.includes("invalid email") || errorMessage.includes("email format is invalid")) {
    return {
  ...
  // Too many requests
  if (errorMessage.includes("too many requests") || errorMessage.includes("rate limit") || status === 429) {
    return {
