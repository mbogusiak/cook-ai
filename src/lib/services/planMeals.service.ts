...
export async function getPlanMealById(
  ...
  const { data, error } = await supabase.from("plan_meals").select("*").eq("id", planMealId).maybeSingle();
 ...
 * @throws ServerError on database errors
 */
export async function getRecipeById(recipeId: number, supabase: SupabaseClient<Database>): Promise<RecipeData | null> {
...
export async function performSwapTransaction(
  ...
  try {
    ...
    const { data: updatedMeals, error: updateError } = await supabase
      ...
      .in("id", mealsToUpdate)
      .select(
        "id, slot, status, calories_planned, portion_multiplier, multi_portion_group_id, is_leftover, recipe_id, updated_at"
      );
    ...
    const portionsMap = new Map((mealsWithPortions || []).map((m) => [m.id, m.portions_to_cook as number | null]));
    ...
    throw new ServerError("Failed to perform swap transaction", error instanceof Error ? error : undefined);
  }
}
...
export async function getAlternativesForMeal(
  ...
  try {
    ...
    const targetCaloriesPerServing = Math.round(planMeal.calories_planned / planMeal.portion_multiplier);
    ...
    throw new ServerError("Failed to fetch meal alternatives", error instanceof Error ? error : undefined);
  }
}
