...
import {
  ...
  getAlternativesForMeal,
  updateMealStatus,
} from "./planMeals.service";
...
function createSupabaseStubForPlanMeals() {
  ...
  const plan_meals: any[] = [
    ...
    // Group meals (owned by u1) dinner pair
    {
      ...
      calories_planned: 700,
      multi_portion_group_id: "grp",
      is_leftover: false,
      portions_to_cook: 4,
      created_at: "",
      updated_at: "",
  ...
  const builder: any = {
    ...
    single() {
      const r = this._execSelect(true);
      resetQuery();
      return { data: Array.isArray(r.data) ? r.data[0] : r.data, error: r.error };
    },
    maybeSingle() {
      const r = this._execSelect(false);
      resetQuery();
      return { data: Array.isArray(r.data) ? r.data[0] : r.data, error: r.error };
    },
    async _resolve() {
      const r = this._execSelect();
      resetQuery();
      return r;
    },
    then(resolve: any) {
    ...
    _execSelect(isSingle?: boolean) {
      ...
      if (t === "plan_meals") {
        if (state.updatePayload) {
          const ids =
            (state.query["in_id"] as number[]) || (state.query["id"] !== undefined ? [state.query["id"]] : []);
          ...
          if (state.selectFields === "id") {
            if (state.query["multi_portion_group_id"]) {
              data = plan_meals
                .filter((m) => m.multi_portion_group_id === state.query["multi_portion_group_id"])
                .map((m) => ({ id: m.id }));
            } else if (state.query["in_id"]) {
              data = plan_meals
                .filter((m) => (state.query["in_id"] as number[]).includes(m.id))
                .map((m) => ({ id: m.id }));
            ...
            data = plan_meals
              ...
              .map((m) => ({ id: m.id, portions_to_cook: m.portions_to_cook }));
      ...
      if (t === "recipes") {
        ...
        if (state.query["gte_calories_kcal"] !== undefined)
          rows = rows.filter((r) => r.calories_kcal >= state.query["gte_calories_kcal"]);
        if (state.query["lte_calories_kcal"] !== undefined)
          rows = rows.filter((r) => r.calories_kcal <= state.query["lte_calories_kcal"]);
        ...
        if (rows.length === 0 && state.query["id"] !== undefined) {
          data = null;
        } else {
          data = rows;
        }
      ...
      if (t === "plan_day_slot_targets") {
        const row = plan_day_slot_targets.find(
          (s) => s.plan_day_id === state.query["plan_day_id"] && s.slot === state.query["slot"]
        );
        data = row || null;
      }
      return { data, error };
  ...
  return {
    ...
    tables: { plan_meals, recipes, recipe_slots, plan_day_slot_targets },
  };
}
...
describe("planMeals.service - validateSwapCandidate", () => {
  it("fails when recipe not available for slot", async () => {
    ...
    const recipe = {
      ...
      calories_kcal: 300,
      portions: 2,
      available_slots: ["breakfast"] as Enums<"meal_slot">[],
    };
