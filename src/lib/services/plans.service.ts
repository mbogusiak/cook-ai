 ...
 */
function calculateSlotTargets(dailyCalories: number): Record<Enums<"meal_slot">, number> {
  return {
...
async function selectRecipeWithFallback(
  ...
  let recipes = await getRecipesForSlot(supabase, slot, targetCalories, excludeRecipeIds, 0.2);
  ...
  if (recipes.length === 0) {
    ...
    recipes = await getRecipesForSlot(supabase, slot, targetCalories, excludeRecipeIds, 0.3);
  ...
  if (recipes.length === 0) {
    ...
    recipes = await getRecipesForSlot(supabase, slot, targetCalories, [], 0.3);
  ...
  if (recipes.length === 0) {
    console.error(
      `[selectRecipeWithFallback] FAILED: No recipes available at all for ${slot} (target: ${targetCalories} cal)`
    );
    return null;
  }
 ...
 */
async function checkExistingActivePlan(supabase: SupabaseClient<Database>, userId: string): Promise<boolean> {
  const { data, error } = await supabase
    .from("plans")
    .select("id")
    .eq("user_id", userId)
...
export async function getPlans(
  ...
  try {
    ...
    const { data: allPlans, error: allPlansError } = await supabase.from("plans").select("state").eq("state", "active");
...
export async function generatePlan(
  ...
  if (hasActivePlan) {
    throw new ConflictError("User already has an active plan. Archive or complete the existing plan first.");
  }
  ...
  try {
    ...
    const { error: slotTargetsError } = await supabase.from("plan_day_slot_targets").insert(slotTargetsToInsert);
 ...
 * @throws ServerError on database errors
 */
export async function getPlanDetailsWithMeals(supabase: SupabaseClient<Database>, planId: number, userId: string) {
  try {
    ...
    const recipeSlots = new Map<number, string[]>();
...
export async function getPlanById(
  ...
  try {
    ...
    let targetsData: any[] = [];
    if (dayIds.length > 0) {
      const { data, error } = await supabase.from("plan_day_slot_targets").select("*").in("plan_day_id", dayIds);
    ...
    if (mealsData.length > 0) {
      ...
      const { data, error } = await supabase.from("recipe_slots").select("recipe_id, slot").in("recipe_id", recipeIds);
