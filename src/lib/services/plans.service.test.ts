...
function createSupabaseStubForGeneratePlan(options?: {
  recipes?: { id: number; name: string; calories_kcal: number; portions: number; slots: Enums<"meal_slot">[] }[];
}) {
    ...
    const supabase: any = {
      from(table: string) {
        const api: any = {
          ...
          _makeRecipeSlots() {
            return [
              { recipe_id: recipeId, slot: "lunch" },
              { recipe_id: recipeId, slot: "dinner" },
            ];
          },
          async maybeSingle() {
            return { data: null, error: null };
