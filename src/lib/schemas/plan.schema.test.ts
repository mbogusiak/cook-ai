...
describe('plan schemas - createPlanCommandSchema', () => {
  ...
  it('rejects invalid calories and length ranges', () => {
    const future = isoDate(2)
    expect(() => createPlanCommandSchema.parse({ daily_calories: 799, plan_length_days: 7, start_date: future })).toThrow()
    expect(() => createPlanCommandSchema.parse({ daily_calories: 6001, plan_length_days: 7, start_date: future })).toThrow()
    expect(() => createPlanCommandSchema.parse({ daily_calories: 2000, plan_length_days: 0, start_date: future })).toThrow()
    expect(() => createPlanCommandSchema.parse({ daily_calories: 2000, plan_length_days: 366, start_date: future })).toThrow()
  })

  it('rejects invalid or non-future start_date', () => {
    expect(() => createPlanCommandSchema.parse({ daily_calories: 2000, plan_length_days: 7, start_date: 'invalid' })).toThrow()
    // definitely in the past
    expect(() => createPlanCommandSchema.parse({ daily_calories: 2000, plan_length_days: 7, start_date: '2000-01-01' })).toThrow()
  })
})
